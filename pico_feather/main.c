...
const uint32_t ADC_DATA_DEPTH = 32*1024; //(32k)
...
void SSD1306_send_cmd(uint8_t cmd) {
    ...
    uint8_t disp_buf[2] = {0x80, cmd};
    i2c_write_blocking(I2C_PORT, SSD1306_I2C_ADDR, disp_buf, 2, false);
}

void SSD1306_send_cmd_list(uint8_t *disp_buf, int num) {
    for (int i=0;i<num;i++)
        SSD1306_send_cmd(disp_buf[i]);
}

void SSD1306_send_buf(uint8_t disp_buf[], int buflen) {
    ...
    temp_buf[0] = 0x40;
    memcpy(temp_buf+1, disp_buf, buflen);
...
void render(uint8_t *disp_buf, struct render_area *area) {
...
static void SetPixel(uint8_t *disp_buf, int x,int y, bool on) {
...
// Basic Bresenhams.
static void DrawLine(uint8_t *disp_buf, int x0, int y0, int x1, int y1, bool on) {
...
static void WriteChar(uint8_t *disp_buf, int16_t x, int16_t y, uint8_t ch) {
    if (reversed[0] == 0) 
        FillReversedCache();
    ...
    for (int i=0;i<8;i++) {
        disp_buf[fb_idx++] = reversed[idx * 8 + i];
    }
}

static void WriteString(uint8_t *disp_buf, int16_t x, int16_t y, char *str) {
    // Cull out any string off the screen
    if (x > SSD1306_WIDTH - 8 || y > SSD1306_HEIGHT - 8)
        return;

    while (*str) {
        WriteChar(disp_buf, x, y, *str++);
        x+=8;
    }
}
...
int main() {
    ...
    while(true){
        ...
        if ((curr_disp_millis - previous_disp_millis) > 50){
            previous_disp_millis = curr_disp_millis;
            memset(disp_buf, 0, SSD1306_BUF_LEN);

            // update to use the csr write_addr
            ...
            float disp_freq = 2*test_freq;
            float sample_rate = 500000.0;
            float step = sample_rate / (disp_freq * 128);
            ...
            // char text[20] = {0};
            // sprintf(text, "%d", transfer_count);
            // WriteString(disp_buf, 5, 0, text); 

            // sprintf(text, "%d", disp_counter);
            // WriteString(disp_buf, 5, 18, text); 
